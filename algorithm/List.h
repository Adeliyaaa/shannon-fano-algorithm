...
public:
	...
	void plus(List_node* cur, bool zero)
	{
		if (zero == true)
			cur->code += "0";
		else
	...
	void encoding(int index1, int index2) {
		int sum = 0;
		List_node* cur = Head;
		...
		if (index1 == index2)
		...
		else if (index2 - index1 == 1 && get_pointer(index1)->washere == false && get_pointer(index2)->washere == false) { //idut podryad
			plus(get_pointer(index1), true);
			plus(get_pointer(index2), false);
			get_pointer(index1)->washere = true;
			get_pointer(index2)->washere = true;
			...
			for (int i = index1; i <= index2; i++)
			{
				cursum += get_pointer(i)->data;

				if (cursum <= semisum)
					if (get_pointer(i)->washere == false)
						plus(get_pointer(i), true);
				if (cursum > semisum){
					if (isp < 0) isp = i;
					if (get_pointer(i)->washere == false)
					plus(get_pointer(i), false);
				}

			}

			if (isp < 0) isp = index2 - 1;
			encoding(index1, isp - 1);
			encoding(isp, index2);
		}
